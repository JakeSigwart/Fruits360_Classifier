...
with tf.Session(graph=graph) as sess:
	...
	fruit_data.load_dataset(training_set=True)
	
	#TRAINING LOOP
	accuracy_plot = np.zeros([num_batches], dtype=np.float32)
	for n in range(0, num_batches):
		...
		#Run session
		acc_output, gradients, predictions_out  = sess.run([accuracy, optimizer, predictions], feed_dict={Images:images, Labels:hot_labels, Training_status:True})		
		print('Batch: '+str(n)+ '  Accuracy: '+str(acc_output))
		accuracy_plot[n] = acc_output
	...
	#Save tensorflow session, save embeddings
	if save_run:
		print('Saving model...')
